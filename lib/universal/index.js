...
class UniversalPark extends Park {
    ...
    constructor(options = {}) {
        ...
        options.scheduleDaysToReturn = 365;
        ...
        return new Promise(function(resolve, reject) {
            ...
            this.GetAPIUrl({
                ...
                for (var i = 0, day; day = body[i++];) {
                    if (day.VenueStatus === 'Closed Due To Weather') {
                        this.Schedule.SetDate({
                            date: Moment.tz(day.OpenTimeString, "YYYY-MM-DDTHH:mm:ssZ", this.Timezone),
                            type: "Closed"
                        });
                    } else {
                        this.Schedule.SetDate({
                    ...
                    if (day.EarlyEntryString && day.EarlyEntryString != day.OpenTimeString) {
                        this.Schedule.SetDate({
                            openingTime: Moment.tz(day.EarlyEntryString, "YYYY-MM-DDTHH:mm:ssZ", this.Timezone),
                            closingTime: Moment.tz(day.OpenTimeString, "YYYY-MM-DDTHH:mm:ssZ", this.Timezone),
                            specialHours: true, 
                            type: 'Early Entry'
                        });
                    }
